			...
			switch(m) {
				case "u": {	// 파일 업로드
			    	 ...
			    	 if(!token[0].equals("") && token[0]!=null) {
						  boolean exist = fi.fileExist(token[0]);
						  if(exist) {	// 파일 존재 여부 확인 -> 성공시
								byte[] bt = fi.upload(token[0]);
								long len = fi.size(token[0]);
								prq = new PDSRequest();
								pv = new PDSVO();
								pv.fname = token[0];
								pv.author = token[1];
				...
				case "i": {	// 상세보기
			    	...
			    	for(int i = 0; i < list.size(); i++) {
			    		System.out.println(list.get(i) + " " + list.get(i).desc);
				...
				case "f": {	// 파일명으로 검색
			    	...
			    	int idx = list.indexOf(pResp.pv);
			    	System.out.println(list.get(idx) + " " + list.get(idx).desc);
			    	String str = new String(list.get(idx).fdata, "UTF-8");
			    	System.out.println(str);
			    	break;
				}
				case "m": {	// 파일 수정
